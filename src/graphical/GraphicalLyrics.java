...
public class GraphicalLyrics {
	...
	private void drawLines(Graphics g, List<GraphicalNote> notes) {
		for (int i = 0; i < notes.size() - 1; i++) {
			...
			Note n1 = gn1.getNote();
			if (!n1.hasLyricExtender() && !n1.hasSyllableDivision()) {
				continue;
			}
		    ...
		    int baseY = gScore.getStaff(staffIndex).getYPosOfLine(0) + 20;     // sotto il pentagramma
			if (n1.hasLyricExtender()) {
				drawLyricExtender(g, x1, x2, baseY);
			}
			if (n1.hasSyllableDivision()) {
				drawSyllableDivision(g, x1, x2, baseY);
			}
		}
	}
